...
** Session
    ...
    So like in [[sec:config][Sample configuration]], you have to provide the =:session= header argument.
...
** Asynchronous evaluation
   ...
   But usually it blocks user inputs. In [[sec:config][Sample Configuration]], =C-c C-c= is bound to =ob-sagemath-ctrl-c-ctrl-c=.
...
#+BEGIN_EXAMPLE
  ,#+BEGIN_SRC sage :file sin.png
    # You can change the size of the image by the figsize argument.
